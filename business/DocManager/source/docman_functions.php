...
function getBreadcrumbsByID($folderid) 
{
		...
		if( $curitem = db_fetch_array($list)) 
		{
			if(in_array($folderid, $_SESSION["path_folder"])){
				if($dropdown)
					...
					$hash = $_SERVER[PHP_SELF]."?hash=".$curitem["hash"];
	...
	if( $_SESSION["UserID"] != "Guest" )
		$str = "<a class='dropdown' href=".$_SERVER[PHP_SELF]."?folder=0>".ROOT_FOLDER."</a>".$str.$str2;
	else
...
function getSearchPath($fid){
	if($fid==0){
		...
		$fpath = "";
		while( $fid > 0 ){
			$strSQL = "select * from doc_files where id=".$fid;
